...
@section('content')
    ...
    <div class="page">
        ...
        <div class="top-controls">
            ...
            <div class="controls-right">
                ...
                <a href="{{ route('mitra.allproject_download', ['start' => request('start'), 'end' => request('end')]) }}"
                ...
                <tbody>
                    @foreach ($project_doc as $index => $project)
                        <tr>
                            ...
                            <td>
                                <a href="
                                    @if ($project['status'] === 'PROCESS') {{ route('mitra.allproject_process_detail', $project['id']) }}
                                    @elseif (
                                        $project['status'] === 'ACC' ||
                                            $project['status'] === 'REKONSILIASI' ||
                                            $project['status'] === 'REVIEW TA' ||
                                            $project['status'] === 'CLOSE') {{ route('mitra.allproject_acc_detail', $project['id']) }}
                                    @elseif ($project['status'] === 'REJECT') {{ route('mitra.allproject_reject_detail', $project['id']) }}
                                    @else {{ route('mitra.allproject', $project['id']) }} @endif "
                                    title="Lihat Detail">
    ...
    <div id="addProjectModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <form class="addProjectForm" id="addProjectForm" action="{{ route('mitra.allproject_create') }}" method="POST"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (start && end && downloadBtn) {
                downloadBtn.href = "{{ route('mitra.allproject_download') }}" + `?start=${start}&end=${end}`;
            }
                            ...
                            Swal.fire({
                                ...
                                if (status === "PROCESS") {
                                    window.location.href =
                                        "{{ route('mitra.process') }}"; // misal halaman default PROCESS
                                } else if (status === "ACC") {
                                    window.location.href =
                                        "{{ route('mitra.acc') }}";
